...
int Ball::findPlatform(std::deque<Platform *> platforms)
{
    standingOn = nullptr;

    for (int i = 0; i < platforms.size(); i++)
    {
        if (circle.getPosition().y >= platforms[i]->sprite.getPosition().y && circle.getPosition().y <= platforms[i]->sprite.getPosition().y + 10)
