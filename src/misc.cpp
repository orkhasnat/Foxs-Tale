...
int menu()
{
    ...
    while (window.isOpen())
    {
        while (window.pollEvent(event))
        {
            ...
            {
                for (i = 0; i < 5; i++)
                    if (text[i].getGlobalBounds().contains(XCursor, YCursor))
                    {
                        text[active].setFillColor(inactive);
                        active = i;
                        break;
            ...
            {
                for (i = 0; i < 5; i++)
                    if (text[i].getGlobalBounds().contains(XButton, YButton))
                    {
                        text[active].setFillColor(inactive);
                        active = i;
                        return active;
...
void readrecords(std::set<std::pair<int, std::string>> &records, std::ifstream &fin)
{
    ...
    //while(records.size() > 10) records.erase(records.begin()); the reason behind the garbage values.
}
...
void displayrecords(const std::set<std::pair<int, std::string>> records)
{
    ...
    {
        score[i][0].setFont(arial);
        score[i][1].setFont(arial);
        score[i][0].setCharacterSize(30);
        score[i][1].setCharacterSize(30);
        score[i][0].setFillColor(sf::Color::Yellow);
        score[i][1].setFillColor(sf::Color::Yellow);
        score[i][0].setOutlineThickness(2);
        score[i][1].setOutlineThickness(2);
        score[i][0].setOutlineColor(sf::Color::Black);
        score[i][1].setOutlineColor(sf::Color::Black);
        score[i][0].setStyle(sf::Text::Bold);
        score[i][1].setStyle(sf::Text::Bold);
        score[i][0].setPosition(810, 110 + 60 * i);
        score[i][1].setPosition(1060, 110 + 60 * i);

        score[i][0].setString(it->second);
        score[i][1].setString(std::to_string(it->first));
    }
